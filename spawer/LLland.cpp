...
LangPack LangP("plugins/spawner/langpack/land.json");
...
#define LAssert(x,y) (x)&&__fail(y,__LINE__)
	...
	static inline void INITDB() {
		db = MakeKVDB(GetDataPath("spawner"), true, 4, 10);
	}
};
...
static inline void NoticePerm(WPlayer wp, FastLand const*) {
	wp.sendText(u8"§l§6[§eMCBE §bFINE§6]§r "+ _TRS("land.no.perm"), RAW);
}
static void loadConfig() {
	try {
		ConfigJReader jr("plugins/spawner/land.json");
...
void entry() {
	...
	Event::addEventListener([](RegCmdEV ev) {
		...
		MakeCommand("lands", "op land command", 1);
		CmdOverload(lands, oncmd, "point-op");
		CmdOverload(lands, oncmd_2, "land-op");
		CmdOverload(lands, oncmd_3, "land-op2", "target");
		CmdOverload(lands, oncmd_perm, "perm", "land_perm");
		MakeCommand("land_ops", "land op", 1);
		CmdOverload(land_ops, oncmd_landop,"op","name");
		});
...
THook(void, "?handle@ServerNetworkHandler@@UEAAXAEBVNetworkIdentifier@@AEBVItemFrameDropItemPacket@@@Z", ServerNetworkHandler* thi, NetworkIdentifier& a2, unsigned char* pk) {
	...
	if (sp) {
		...
		auto fl = getFastLand(x, z, wp.getDimID());
		if (fl) {
			if (wp->getCommandPermissionLevel() == 0) {
				NoticePerm(wp, fl);
				return;
			}
		}
...
THook(bool, "?mayPlace@BlockSource@@QEAA_NAEBVBlock@@AEBVBlockPos@@EPEAVActor@@_N@Z",
	...
	if (pl) {
		...
		auto fl = getFastLand(blkpos->x, blkpos->z, wp.getDimID());
		if (fl) {
			if (wp->getCommandPermissionLevel() == 0) {
				NoticePerm(wp, fl);
				return false;
...
THook(bool, "?_hurt@Mob@@MEAA_NAEBVActorDamageSource@@H_N1@Z", Mob*_this, ActorDamageSource* ads, int damage, bool unk1_1, bool unk2_0) {
	...
	if (_this->getPos().y < LAND_MIN_Y) return false;
